...
Proof.
(**
 intros a n. unfold iff. refine (conj _ _).
*)
 ...
 - (* -> *)
   ...
   + (* E_AMult *)
     rewrite IHaevalR1.  rewrite IHaevalR2.  reflexivity.
 - (* <- *)
   (** revert n would work the same in this case, 
       basically brings back n from the context to the goal. Basically the opposite of 'intros n' *)
   ...
   + (* ANum *)
     apply E_ANum.
   + (* APlus *)
(** Bahman's snippet using refine: 

E_APlus takes 6 arguments: (e1 e2: aexp) (n1 n2: nat) and 
two proofs: proof_of_aevalR_e1_n1 and proof_of_aevalR_e2_n2.

    e1 \\ n1  <---- proof_of_aevalR_e1_n1
    e2 \\ n2  <---- proof_of_aevalR_e2_n2

--------------------- (E_APlus)
APlus e1 e2 \\ n1+n2

So we have e1 is a1, e2 is a2, n1 is (aeval a1), n2 is (aeval a2), 
and we need two proofs but we leave those last two arguments as _ in the refine tactic:


     refine (E_APlus a1 a2 (aeval a1) (aeval a2) _ _).
     * (* a1 \\ aeval a1 *)
...
(** Bahman: added a 'simpl' to the following two to make the use of reflexivity clear *)
Example aexp1 :
  aeval { X --> 5 } (3 + (X * 2))
  = 13.
Proof.  
...
(* Concatenating two dictionaries is necessary to load inside IF and WHILE statements.
   If we have overlapping entries, we default to the entry in the second argument.

   This is like saying, `the latest instance of the address is the canonical one`, though
   good programming practice is to not do this in the first place. *)

Fixpoint lc_concat (tlc1 : lc) (tlc2 : lc) : lc :=
  match tlc2 with
  | []                => tlc1
  | (adr, c) :: tlc2' => lc_update adr c (lc_concat tlc1 tlc2')
  end.

Compute lc_concat [(1, SKIP) ; (2, SKIP)] [(3, X ::= 5)].

Compute lc_concat [(1, X ::= 2) ; (2, X ::= 3)] [(2, SKIP) ; (4, SKIP)].
...
(** Here is an informal definition of evaluation, presented as inference
    rules for readability:
                   ...
                   x := a1 / st \\ st & { x --> n }
                     ...
                     ifcom_eval (Ifcom b c1 c2) st st'
                   -------------------------------------                (E_Jump)
                   adr :IF b1 THEN c1 ELSE c2 FI / st \\ st'                        
...
(* W.L. edit:
   Some modifications : a single evaluation type.
   For robustness, we must supply the address-command dictionary as an argument, and the address
   of the If statement must match the address in the dictionary at evaluation.  Think of the dictionary
...
Definition sw_lc := [ (1, Y ::= 2) ; (2, Y ::= 3) ].

Check (SWITCH X sw_lc).
...
(* ** W.L. addition 9/7/18: automatic population of lc dictionary and evaluation ** *)

(* loadDict_outer is the old version that doesn't look inside IF and WHILE statements.

   The new loadDict does look inside If and While statements, with the notion of `the rest of the program`
...
Fixpoint loadDict_outer (c : com) : lc :=
  match c with
  | c1 ;; c2            => match (isAtomicAdr c1) with
                           | true => (lc_update (getAdr c1) (c1 ;; c2) (loadDict_outer c2))
                           | false => loadDict_outer c2
                           end
  | _                   => match (isAtomicAdr c) with
...
(* Function to generate the smallest fresh address, given an existing dictionary. *)

Fixpoint lc_length (tlc : lc) :=
  match tlc with
  | []     => 0
  | h :: t => 1 + lc_length t
  end.

(* The idea is that we start searching at 0; if the address isn't found in the dictionary, we return that value.  
   If it is found, then we add 1 and go again.  However, this may not terminate, which Coq doesn't like.

   So the workaround is this: we use an extra `gas` parameter, that decreases by 1 each time the function recurses.
   We want to set the gas value to the length of the list plus 1.  That way, if gas runs out, we know we've checked
...
Fixpoint no_whiles (c : com) : bool :=
  ...
  | WHILE _ DO _ END  =>
      false
  | adr :WHILE _ DO _ END => 
      false
  | _ => false (** Bahman: Just so it compiles, otherwise this is NOT right *)
  end.
